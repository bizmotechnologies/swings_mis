...
//------------this fun is used to get all customer details------------//
  public function sort_Enquiries(){
    extract($_POST);
    $path = base_url();
    $url = $path . 'api/Sort_Enquiry_Quotations_api/sort_Enquiries?Enquiry_From_date='.$Enquiry_From_date.'&Enquiry_To_date='.$Enquiry_To_date.'&customer_id='.$customer_idEnquiryFilter;
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_HTTPGET, true);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $response_json = curl_exec($ch);
    ...
    if($response['status']==0){
      echo '<tr>
      <td colspan="6"><div class="alert alert-danger">
