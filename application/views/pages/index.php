...
<body style="background-color: white">
  ...
  <div class="w3-col l4 w3-padding-xxlarge">
    ...
    <form class="w3-border-bottom w3-small" id="login_form">
     ...
     <label for="login_user" class="w3-label">Select Branch:</label>
     <select name="login_branch" class="form-control w3-margin-bottom" required>
       <option class="active w3-red" value="0">Select branch</option>
        <?php 
        if(isset($all_branches['status'])){
          echo '<option>'.$all_branches['status_message'].'</option>';                    
        }
        else{
          foreach ($all_branches as $key) {                  
           echo '<option value="'.$key['branch_name'].'">'.$key['branch_name'].'</option>';
         }
       }
       ?>
     </select>
     ...
     <button class="btn btn-md w3-red btn-block w3-margin-bottom" name="login_submit" id="login_submit" type="submit">Sign in</button>
